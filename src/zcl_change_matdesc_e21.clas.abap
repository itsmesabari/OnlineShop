CLASS zcl_change_matdesc_e21 DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS zcl_change_matdesc_e21 IMPLEMENTATION.
...
* The following code can be used to update description of an existing product
    MODIFY ENTITIES OF I_ProductTP_2
        ...
        WITH VALUE #( ( %key-Product = 'DUMMY_PRODUCT_E21'
                        %key-Language = 'E'
                        ProductDescription = 'Dummy Product e21- Updated') )
        REPORTED DATA(lreported)
        FAILED DATA(lfailed).

    COMMIT ENTITIES
      RESPONSE OF I_ProductTP_2
          FAILED DATA(lfailed_commit)
          REPORTED DATA(lreported_commit).
